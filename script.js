...
const selectedProductContainerTemplate = document.getElementById(
...
const totalPriceSpanInConfirmedOrderCard = document.querySelector(
  "span.total-in-confirmed-order-card",
);
...
const orderMiniCardTemplate = document.getElementById(
  "order-mini-card-template",
);
...
startNewOrderBtn.addEventListener("click", () => {
  ...
  document.querySelectorAll("li.product-box").forEach((productBox) => {
    ...
    productBox
      .querySelector("div.control-quantity-buttons-container")
      .classList.add("hidden");
    productBox
      .querySelector("button.add-to-cart-btn")
...
confirmOrderBtn.addEventListener("click", () => {
  ...
  cartArray.forEach((orderObj) => {
    if (orderObj.quantityInCart > 0) {
      const orderMiniCard =
        orderMiniCardTemplate.content.cloneNode(true).firstElementChild;
      orderMiniCard.querySelector("span.order-name").textContent =
        orderObj.productName;
      orderMiniCard.querySelector("span.order-quantity").textContent =
        orderObj.quantityInCart + "x";
      orderMiniCard.querySelector("span.order-price").textContent =
        "@$" + number(orderObj.productPrice);
      orderMiniCard.querySelector("span.order-total-price").textContent =
        "$" + number(orderObj.productPrice * orderObj.quantityInCart);
      displayTotalPrice();
      ordersList.append(orderMiniCard);
      setImage(
        orderObj.productName,
        orderMiniCard.querySelector("img"),
        "thumbnail",
      );
    }
  });
...
function renderProductsElements(productsArray) {
  ...
  productsArray.forEach((productInfoObject) => {
    ...
    productBox.querySelector(".product-img").src =
      productInfoObject.image.mobile;
    productBox.querySelector("span.category").textContent =
      productInfoObject.category;
    productBox.querySelector("span.product-name").textContent =
      productInfoObject.name;
    productBox.querySelector("span.product-price").textContent =
      "$" + number(productInfoObject.price);
    ...
    const controlQuantityButtonsContainer = productBox.querySelector(
      "div.control-quantity-buttons-container",
    );
    const controlQuantityButtons =
      controlQuantityButtonsContainer.querySelectorAll("button");
    const quantitySpan = controlQuantityButtonsContainer.querySelector(
      ".quantity-to-add-span",
    );

    // creating a container in the cart for the element using a template
    const selectedProductContainer =
      selectedProductContainerTemplate.content.cloneNode(
        true,
      ).firstElementChild;
    selectedProductContainer.querySelector(
      ".product-name-in-cart",
    ).textContent = productInfoObject.name;
    selectedProductContainer.querySelector(".product-quantity").textContent =
      productObjectForCart.quantityInCart + "x";
    selectedProductContainer.querySelector(
      ".selected-product-price",
    ).textContent = `@ $${number(productObjectForCart.productPrice)}`;
    selectedProductContainer
      .querySelector("button.remove-product-btn")
      .addEventListener("click", () => {
        productObjectForCart.quantityInCart = 0;
        selectedProductContainer.remove();
        actualBox.classList.remove("red-border");
        actualBox
          .querySelector(".control-quantity-buttons-container")
          .classList.add("hidden");
    ...
    function decreaseQuantity() {
      if (productObjectForCart.quantityInCart > 1) {
        ...
        selectedProductContainer.querySelector(
          ...
          `$${number(productObjectForCart.productPrice * productObjectForCart.quantityInCart)}`;
        selectedProductContainer.querySelector(
          ".product-quantity",
        ).textContent = productObjectForCart.quantityInCart + "x";
        selectedProductContainer.querySelector(
          ".selected-product-price",
        ).textContent = `@ $${number(productObjectForCart.productPrice)}`;
      } else {
